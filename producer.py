...
def process_subjects(subject_set, spark, device_name, device_id, base_dir='./data/raw/wisdm-dataset/raw'):
    ...
    for subj_id in subject_set:
        ...
        try:
            accel_df = read_sensor_file(spark, accel_path, schema=df_schema)
            gyro_df = read_sensor_file(spark, gyro_path, schema=df_schema)
            ...
            for activity in activities:
                ...
                timestamps = create_common_timestamp_grid(start, end, freq_hz=FREQ_HZ)
                accel_i = interpolate_sensor_block(df=accel_pdf, timestamps=timestamps)
                gyro_i = interpolate_sensor_block(df=gyro_pdf, timestamps=timestamps)
...
class DataProducer:
    def __init__(self, bootstrap_servers='localhost:9092', topic_name='wisdm_predictions'):
...
async def main():
    ...
    3. Call send_data() to push everything into `wisdm_predictions`.
    """
